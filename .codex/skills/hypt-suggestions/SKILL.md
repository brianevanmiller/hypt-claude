...
---
<!-- Generated from plugin/skills/suggestions/SKILL.md. Do not edit by hand. Run `node scripts/sync-codex-support.mjs` instead. -->

# hypt-suggestions — Suggest Next Tasks and Update Backlog
...
- **"1" (always update):**
  ```bash
  "$REPO_ROOT"/bin/hypt-config set suggestions_auto_backlog auto
  ```

- **"2" (always ask):**
  ```bash
  "$REPO_ROOT"/bin/hypt-config set suggestions_auto_backlog ask
  ```

- **"3" (skip):**
  ```bash
  "$REPO_ROOT"/bin/hypt-config set suggestions_auto_backlog skip
  ```
