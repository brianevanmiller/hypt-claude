...
MATT_GRILL=$(test -f ~/.claude/skills/grill-me/SKILL.md && echo "true" || echo "false")
MATT_GUARDRAILS=$(test -f ~/.claude/skills/git-guardrails-claude-code/SKILL.md && echo "true" || echo "false")
echo "MATT_SKILLS: grill=$MATT_GRILL guardrails=$MATT_GUARDRAILS"
```
...
- `MATT_SKILLS` line: note silently which of Matt Pocock's companion skills are already installed. Do not mention this to the user yet (Phase 0c will handle the recommendation). If either `MATT_*` flag is missing, Phase 0c will offer to install them.
...
Format as: `> Extras: \`/todo\` (capture an idea) · \`/cso\` (security audit) · ...`
