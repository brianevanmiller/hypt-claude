...
# /plan-critic — Review a Plan Before Building
...
- If the plan references specific files, spot-check that they exist and contain what the plan assumes (use Grep/Glob)
...
2. **File verification:** Do the files and functions mentioned in the plan actually exist in the codebase? (Use Grep/Glob to spot-check the key ones)
...
Use the Agent tool to launch BOTH agents in a SINGLE message (parallel execution). Provide each agent with: the full plan text, the original user request, and the current branch name.
